...
class GameFrameBuilder(alloc: ByteBufAllocator, opcode: Int = -1, type: GameFrame.Type = GameFrame.Type.RAW) {
    private val opcode: Int
    private val type: GameFrame.Type
    val allocator: ByteBufAllocator
    private val buffer: ByteBuf
    private var mode = AccessMode.BYTE_ACCESS
    private var bitIndex = 0

    constructor(alloc: ByteBufAllocator, opcode: Int) : this(alloc, opcode, GameFrame.Type.FIXED)
    ...
    fun toGameFrame(): GameFrame {
        check(type != GameFrame.Type.RAW) { "Raw builders cannot be converted to frames" }

        check(mode == AccessMode.BYTE_ACCESS) { "Must be in byte access mode to convert to a packet" }

        return GameFrame(opcode, type, buffer)
    }
    ...
    fun putRawBuilder(builder: GameFrameBuilder) {
        checkByteAccess()
        require(builder.type == GameFrame.Type.RAW) { "Builder must be raw!" }
