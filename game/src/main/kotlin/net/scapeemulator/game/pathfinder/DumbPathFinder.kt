...
object DumbPathFinder {
    fun find(position: Position, dest: Position, size: Int, max: Int, inside: Boolean): Path? {
        val path = Path()
        var p: Position? = position
        for (i in 0 until max) {
            p = bestDummyPosition(p!!, dest, size)//todo p!!
            if (p != null) {
                if (!inside && p == dest) {
                    val cur = if (path.isEmpty) position else path.points.last
    ...
    private fun bestDummyPosition(cur: Position, next: Position, size: Int): Position? {
        if (cur == next) return null
        val deltaX: Int = next.x - cur.x
        val deltaY: Int = next.y - cur.y
        when (Direction.between(cur, next)) {
            Direction.NONE -> return null
            Direction.NORTH -> return Position(cur.x, cur.y + 1, cur.height)
            Direction.SOUTH -> return Position(cur.x, cur.y - 1, cur.height)
            Direction.EAST -> return Position(cur.x + 1, cur.y, cur.height)
            Direction.WEST -> return Position(cur.x - 1, cur.y, cur.height)
            Direction.NORTH_EAST -> {
                ...
                else if (Direction.isTraversable(cur, Direction.NORTH, size))
                    Position(cur.x, cur.y + 1, cur.height)
                else
                    Position(cur.x + 1, cur.y, cur.height)
            }
            ...
            Direction.SOUTH_EAST -> {
                ...
                return if (abs(deltaX.toDouble()) > abs(deltaY.toDouble()))
                    if (Direction.isTraversable(cur, Direction.EAST, size))
                        ...
                        Position(cur.x, cur.y - 1, cur.height)
                else if (Direction.isTraversable(cur, Direction.SOUTH, size))
                    Position(cur.x, cur.y - 1, cur.height)
                else
                    Position(cur.x + 1, cur.y, cur.height)
            }
