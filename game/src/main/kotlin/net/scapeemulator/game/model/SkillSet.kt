...
class SkillSet {
    ...
    fun size(): Int = level.size
    fun getCurrentLevel(skill: Int): Int = level[skill]
    fun getMaximumLevel(skill: Int): Int = getLevelFromExperience(exp[skill])
    fun getExperience(skill: Int): Double = exp[skill]
    ...
    val combatLevel: Int
        get() {
            ...
            return floor((defence + hitpoints + floor(prayer / 2.0) + floor(summoning / 2.0) + base) / 4.0).toInt()
        }
