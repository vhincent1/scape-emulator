...
class LoginSession(server: GameServer, channel: Channel) : Session(server, channel) {
    ...
    init {
        this.service = server.loginService
        init()
    }
    ...
    fun sendLoginSuccess(status: Int, player: Player) {
        ...
        if (displayMode == 0 || displayMode == 1) player.interfaceSet.displayMode = InterfaceSet.DisplayMode.FIXED
        else player.interfaceSet.displayMode = InterfaceSet.DisplayMode.RESIZABLE
