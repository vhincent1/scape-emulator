...
class UpdateSession(server: GameServer, channel: Channel) : Session(server, channel) {
    ...
    override fun messageReceived(message: Any) {
        if (handshakeComplete) {
            if (message is FileRequest) {
                ...
                val encoder = channel.pipeline().get<XorEncoder>(XorEncoder::class.java)
            ...
            if (status == UpdateStatusMessage.STATUS_OK) {
                ...
                channel.pipeline().remove<ReadTimeoutHandler?>(ReadTimeoutHandler::class.java)
    ...
    companion object {
        private val logger: Logger = LoggerFactory.getLogger(UpdateSession::class.java)
    }
}
