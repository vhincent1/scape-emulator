...
class ItemDefinition {
    ...
    companion object {
        /**
         *
         * @param buffer A [ByteBuffer] that contains information
        ...
        fun decode(buffer: ByteBuffer): ItemDefinition {
            ...
            def.groundOptions = arrayOf<String?>(null, null, "take", null, null)
            def.inventoryOptions = arrayOf<String?>(null, null, null, null, "drop")
            while (true) {
                ...
                if (opcode == 1) def.inventoryModelId = buffer.getShort().toInt() and 0xFFFFF
                    ...
                    def.notedId = buffer.getShort().toInt() and 0xFFFFF
                } else if (opcode == 98) {
                    ...
                    for (index in 0..<len) {
                        val anInt = buffer.getShort().toInt() and 0xFFFFF
                    }
                } else if (opcode == 249) {
                    val length = buffer.get().toInt() and 0xFF
                    for (index in 0..<length) {
                        ...
                        val value: Any? =
                            if (stringInstance) ByteBufferUtils.getJagexString(buffer) else buffer.getInt()
                    }
                }
            }
            return def
